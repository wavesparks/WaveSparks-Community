      ...
      <header className="sticky top-0 z-30 border-b border-[var(--line)] bg-[var(--surface)] backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-7xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:flex-nowrap lg:px-8">
            ...
            {memberLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-2.5 text-sm font-semibold transition duration-150 ease-out active:translate-y-px active:scale-[0.99]",
          ...
          <div className="ml-auto flex min-w-0 items-center justify-end gap-2">
            {viewer?.canAdmin ? (
              <LinkButton href={`/org/${org.slug}/admin/members`} size="sm" variant="ghost">
            ...
            {canInteract ? (
              <>
                <LinkButton
                  ...
                  href={`/org/${org.slug}/compose?kind=feed`}
                  size="sm"
                >
                  <PenLine className="size-4" />
                  Post
                </LinkButton>
                <LinkButton href={`/org/${org.slug}/profile`} size="sm" variant="secondary">
                  <UserCircle2 className="size-4" />
                  <span className="hidden sm:inline">Profile</span>
                </LinkButton>
              </>
            ) : viewer ? (
              <LinkButton href={memberSetupHref(org.slug, viewer)} size="sm">
                <UserCircle2 className="size-4" />
                {viewer.membership.status === "approved" ? "Complete profile" : "Application"}
              </LinkButton>
            ) : (
              <LinkButton href={`/org/${org.slug}/signin`} size="sm">
